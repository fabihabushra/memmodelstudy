...
struct SPSCQueue {
    ...
    bool dequeue(std::uint64_t& value) {
        ...
        // faulty: no wrap on dequeue side
        std::size_t next_head = head_val + 1;
        STORE_HEAD(this, next_head, memory_order_seq_cst);
        return true;
    }
};

struct spsc_sc_fault_dequeue_no_wrap_test
    : rl::test_suite<spsc_sc_fault_dequeue_no_wrap_test, 2> {
    SPSCQueue q;
...
int main() {
    rl::simulate<spsc_sc_fault_dequeue_no_wrap_test>();
    return 0;
}
