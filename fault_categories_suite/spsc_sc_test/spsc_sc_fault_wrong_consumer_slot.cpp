...
struct SPSCQueue {
    ...
    bool dequeue(std::uint64_t& value) {
        ...
        // faulty: read wrong slot
        value = LOAD_SLOT(this, (head_val + 1) % QUEUE_SIZE);
        STORE_HEAD(this, (head_val + 1) % QUEUE_SIZE, memory_order_seq_cst);
        return true;
    }
};

struct spsc_sc_fault_wrong_consumer_slot_test
    : rl::test_suite<spsc_sc_fault_wrong_consumer_slot_test, 2> {
    SPSCQueue q;
...
int main() {
    rl::simulate<spsc_sc_fault_wrong_consumer_slot_test>();
    return 0;
}
