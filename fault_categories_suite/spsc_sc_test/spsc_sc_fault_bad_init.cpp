...
struct SPSCQueue {
    ...
    void init() {
        ...
        // faulty: bad initial state
        STORE_TAIL(this, 1, memory_order_seq_cst);
    ...
    bool enqueue(std::uint64_t value) {
        ...
        RL_ASSERT(tail_val < QUEUE_SIZE);
        RL_ASSERT(head_val < QUEUE_SIZE);
...
struct spsc_sc_fault_bad_init_test
    : rl::test_suite<spsc_sc_fault_bad_init_test, 2> {
    SPSCQueue q;

    void before() {
        q.init();

        // explicit sanity check of initial state
        RL_ASSERT(LOAD_HEAD(&q, memory_order_seq_cst) == 0);
        RL_ASSERT(LOAD_TAIL(&q, memory_order_seq_cst) == 0);
    }
...
int main() {
    rl::simulate<spsc_sc_fault_bad_init_test>();
    return 0;
}
