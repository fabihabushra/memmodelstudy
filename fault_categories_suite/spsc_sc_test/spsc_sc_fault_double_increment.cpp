...
struct SPSCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        ...
        // faulty: advance tail by 2 instead of 1
        std::size_t bad_next_tail = (tail_val + 2) % QUEUE_SIZE;
        STORE_TAIL(this, bad_next_tail, memory_order_seq_cst);
        return true;
    }
...
struct spsc_sc_fault_double_increment_test
    : rl::test_suite<spsc_sc_fault_double_increment_test, 2> {
    SPSCQueue q;
...
int main() {
    rl::simulate<spsc_sc_fault_double_increment_test>();
    return 0;
}
