...
struct MPMCQueue {
    ...
    void init() {
        ...
        for (std::size_t i = 0; i < QUEUE_SIZE; ++i) {
            ...
            // faulty: wrong initial sequence
            STORE_SLOT_SEQ(this, i, 0, memory_order_relaxed);
        }
    }
...
struct mpmc_relaxed_seq_fault_bad_init_test
    : rl::test_suite<mpmc_relaxed_seq_fault_bad_init_test, THREADS> {
    MPMCQueue q;
    ...
    void before() {
        ...
        // explicit sanity checks for initial state
        RL_ASSERT(LOAD_ENQ_POS(&q, memory_order_acquire) == 0);
        RL_ASSERT(LOAD_DEQ_POS(&q, memory_order_acquire) == 0);

        for (std::size_t i = 0; i < QUEUE_SIZE; ++i) {
            RL_ASSERT(LOAD_SLOT_SEQ(&q, i, memory_order_acquire) == i);
        }
    }
...
int main() {
    rl::simulate<mpmc_relaxed_seq_fault_bad_init_test>();
    return 0;
}
