...
struct MPMCQueue {
    ...
    bool dequeue(std::uint64_t& value) {
        ...
        while (true) {
            ...
            // faulty: wrong readiness condition
            if (seq == pos) {
                std::size_t expected = pos;
...
struct mpmc_relaxed_seq_fault_wrong_seq_check_test
    : rl::test_suite<mpmc_relaxed_seq_fault_wrong_seq_check_test, THREADS> {
    MPMCQueue q;
...
int main() {
    rl::simulate<mpmc_relaxed_seq_fault_wrong_seq_check_test>();
    return 0;
}
