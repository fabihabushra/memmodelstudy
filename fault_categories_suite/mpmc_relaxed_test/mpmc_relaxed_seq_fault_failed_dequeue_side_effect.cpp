        ...
        while (true) {
            ...
            if (seq == pos + 1) {
                ...
                if (CAS_DEQ_POS(this, expected, pos + 1,
                    ...
                    STORE_SLOT_SEQ(this, idx, pos + QUEUE_SIZE, memory_order_release);
                    return true;
                }
            } else if (seq < pos + 1) {
                // faulty: failed dequeue has side effects
                value = 777;
                STORE_DEQ_POS(this, pos + 1, memory_order_release);
                return false;
            }
...
struct mpmc_relaxed_seq_fault_failed_dequeue_side_effect_test
    : rl::test_suite<mpmc_relaxed_seq_fault_failed_dequeue_side_effect_test, THREADS> {
    MPMCQueue q;
    ...
    void thread(unsigned tid) {
        if (tid < PRODUCERS) {
            ...
            // probe failure behavior before normal draining
            for (int probe = 0; probe < 2; ++probe) {
                std::size_t old_deq = LOAD_DEQ_POS(&q, memory_order_acquire);
                std::size_t old_enq = LOAD_ENQ_POS(&q, memory_order_acquire);
                ...
                RL_ASSERT(ok == false);
                RL_ASSERT(value == 999);
                RL_ASSERT(LOAD_DEQ_POS(&q, memory_order_acquire) == old_deq);
                RL_ASSERT(LOAD_ENQ_POS(&q, memory_order_acquire) == old_enq);
...
int main() {
    rl::simulate<mpmc_relaxed_seq_fault_failed_dequeue_side_effect_test>();
    return 0;
}
