...
struct MPMCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        ...
        while (true) {
            pos = LOAD_ENQ_POS(this, memory_order_relaxed);  // faulty
            std::size_t idx = pos % QUEUE_SIZE;
            std::size_t seq = LOAD_SLOT_SEQ(this, idx, memory_order_relaxed);  // faulty

            if (seq == pos) {
                std::size_t expected = pos;
                if (CAS_ENQ_POS(this, expected, pos + 1,
                                memory_order_relaxed,   // faulty
                                memory_order_relaxed)) {
                    STORE_SLOT_VALUE(this, idx, value);
                    STORE_SLOT_SEQ(this, idx, pos + 1, memory_order_relaxed);  // faulty
                    return true;
                }
            } else if (seq < pos) {
                return false;
    ...
    bool dequeue(std::uint64_t& value) {
        ...
        while (true) {
            pos = LOAD_DEQ_POS(this, memory_order_relaxed);  // faulty
            std::size_t idx = pos % QUEUE_SIZE;
            std::size_t seq = LOAD_SLOT_SEQ(this, idx, memory_order_relaxed);  // faulty
            ...
            if (seq == pos + 1) {
                ...
                if (CAS_DEQ_POS(this, expected, pos + 1,
                    ...
                    STORE_SLOT_SEQ(this, idx, pos + QUEUE_SIZE, memory_order_relaxed);  // faulty
                    return true;
                }
            } else if (seq < pos + 1) {
                return false;
...
struct mpmc_relaxed_seq_fault_all_relaxed_test
    : rl::test_suite<mpmc_relaxed_seq_fault_all_relaxed_test, THREADS> {
    MPMCQueue q;
...
int main() {
    rl::simulate<mpmc_relaxed_seq_fault_all_relaxed_test>();
    return 0;
}
