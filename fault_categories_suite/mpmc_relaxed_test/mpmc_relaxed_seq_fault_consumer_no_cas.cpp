...
struct MPMCQueue {
    ...
    bool dequeue(std::uint64_t& value) {
        ...
        while (true) {
            ...
            if (seq == pos + 1) {
                // faulty: plain store instead of CAS claim
...
struct mpmc_relaxed_seq_fault_consumer_no_cas_test
    : rl::test_suite<mpmc_relaxed_seq_fault_consumer_no_cas_test, THREADS> {
    MPMCQueue q;
...
int main() {
    rl::simulate<mpmc_relaxed_seq_fault_consumer_no_cas_test>();
    return 0;
}
