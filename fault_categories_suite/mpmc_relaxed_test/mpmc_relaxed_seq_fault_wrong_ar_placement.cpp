...
static constexpr int ITEMS_PER_PRODUCER = 2;
...
struct MPMCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        ...
        while (true) {
            // looks plausible, but the real bug is on slot seq ordering below
            pos = LOAD_ENQ_POS(this, memory_order_acquire);
            std::size_t idx = pos % QUEUE_SIZE;

            // faulty placement: should be acquire on slot seq
            std::size_t seq = LOAD_SLOT_SEQ(this, idx, memory_order_relaxed);
            ...
            if (seq == pos) {
                ...
                if (CAS_ENQ_POS(this, expected, pos + 1,
                    ...
                    // faulty placement: should be release on slot seq publish
                    STORE_SLOT_SEQ(this, idx, pos + 1, memory_order_relaxed);
                    return true;
                }
            } else if (seq < pos) {
                return false;
        ...
        while (true) {
            ...
            if (seq == pos + 1) {
                ...
                if (CAS_DEQ_POS(this, expected, pos + 1,
                    ...
                    // faulty placement: should be release when handing slot back
                    STORE_SLOT_SEQ(this, idx, pos + QUEUE_SIZE, memory_order_relaxed);
                    return true;
                }
            } else if (seq < pos + 1) {
                return false;
...
struct mpmc_relaxed_seq_fault_wrong_ar_placement_test
    : rl::test_suite<mpmc_relaxed_seq_fault_wrong_ar_placement_test, THREADS> {
    MPMCQueue q;
...
int main() {
    rl::simulate<mpmc_relaxed_seq_fault_wrong_ar_placement_test>();
    return 0;
}
