...
struct MPMCQueue {
    ...
    void init() {
        ...
        for (std::size_t i = 0; i < QUEUE_SIZE; ++i) {
            ...
            STORE_SLOT_SEQ(this, i, i, memory_order_seq_cst);
        }
    }
    ...
    bool enqueue(std::uint64_t value) {
        ...
        while (true) {
            ...
            // faulty: write before confirming success
...
struct mpmc_sc_fault_write_before_full_check_test
    : rl::test_suite<mpmc_sc_fault_write_before_full_check_test, THREADS> {
    MPMCQueue q;
    ...
    void snapshot_and_assert_no_change_on_failed_enqueue(std::uint64_t probe_value) {
        std::size_t old_enq = LOAD_ENQ_POS(&q, memory_order_seq_cst);
        std::size_t old_deq = LOAD_DEQ_POS(&q, memory_order_seq_cst);

        std::uint64_t old_values[QUEUE_SIZE];
        std::size_t old_seqs[QUEUE_SIZE];

        for (std::size_t i = 0; i < QUEUE_SIZE; ++i) {
            old_values[i] = LOAD_SLOT_VALUE(&q, i);
            old_seqs[i] = LOAD_SLOT_SEQ(&q, i, memory_order_seq_cst);
        }

        bool ok = q.enqueue(probe_value);
        RL_ASSERT(ok == false);

        RL_ASSERT(LOAD_ENQ_POS(&q, memory_order_seq_cst) == old_enq);
        RL_ASSERT(LOAD_DEQ_POS(&q, memory_order_seq_cst) == old_deq);

        for (std::size_t i = 0; i < QUEUE_SIZE; ++i) {
            RL_ASSERT(LOAD_SLOT_VALUE(&q, i) == old_values[i]);
            RL_ASSERT(LOAD_SLOT_SEQ(&q, i, memory_order_seq_cst) == old_seqs[i]);
        }
    }
    ...
    void thread(unsigned tid) {
        if (tid < PRODUCERS) {
            ...
            // Wait until queue becomes full enough to probe failed enqueue
            if (tid == PRODUCERS) {
                while (true) {
                    std::size_t enq = LOAD_ENQ_POS(&q, memory_order_seq_cst);
                    std::size_t deq = LOAD_DEQ_POS(&q, memory_order_seq_cst);
                    if ((enq - deq) >= (QUEUE_SIZE - 1)) {
                        break;
                    }
                    SCHEDULE_POINT();
                }

                snapshot_and_assert_no_change_on_failed_enqueue(9999);
                SCHEDULE_POINT();
            }
...
int main() {
    rl::simulate<mpmc_sc_fault_write_before_full_check_test>();
    return 0;
}
