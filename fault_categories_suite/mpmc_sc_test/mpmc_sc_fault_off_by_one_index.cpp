...
struct MPMCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        ...
        while (true) {
            ...
            // faulty: wrong slot index
            std::size_t idx = (pos + 1) % QUEUE_SIZE;
...
struct mpmc_sc_fault_off_by_one_index_test
    : rl::test_suite<mpmc_sc_fault_off_by_one_index_test, THREADS> {
    MPMCQueue q;
...
int main() {
    rl::simulate<mpmc_sc_fault_off_by_one_index_test>();
    return 0;
}
