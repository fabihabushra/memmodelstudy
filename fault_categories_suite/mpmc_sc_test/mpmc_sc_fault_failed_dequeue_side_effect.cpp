    ...
    bool dequeue(std::uint64_t& value) {
        ...
        while (true) {
            ...
            if (seq == pos + 1) {
                ...
                // faulty: failure has side effects
                value = 777;
                STORE_DEQ_POS(this, pos + 1, memory_order_seq_cst);
                return false;
            }
...
struct mpmc_sc_fault_failed_dequeue_side_effect_test
    : rl::test_suite<mpmc_sc_fault_failed_dequeue_side_effect_test, THREADS> {
    MPMCQueue q;
    ...
    void thread(unsigned tid) {
        if (tid < PRODUCERS) {
            ...
            // Probe failed dequeue behavior before normal draining
...
int main() {
    rl::simulate<mpmc_sc_fault_failed_dequeue_side_effect_test>();
    return 0;
}
