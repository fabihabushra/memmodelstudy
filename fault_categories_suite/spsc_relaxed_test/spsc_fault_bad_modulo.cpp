...
struct SPSCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        ...
        // faulty: no wraparound
        std::size_t next_tail = tail_val + 1;

        // stronger invariant check in harness-facing code
        RL_ASSERT(tail_val < QUEUE_SIZE);
        RL_ASSERT(head_val < QUEUE_SIZE);
        RL_ASSERT(next_tail <= QUEUE_SIZE);

        if ((next_tail % QUEUE_SIZE) == head_val) {
            return false;
        }

        STORE_SLOT(this, tail_val, value);

        // faulty: store non-wrapped tail
    ...
    bool dequeue(std::uint64_t& value) {
        ...
        // faulty: no wraparound
        std::size_t next_head = head_val + 1;
        RL_ASSERT(next_head <= QUEUE_SIZE);

        STORE_HEAD(this, next_head, rl::memory_order_release);
        return true;
    }
};
