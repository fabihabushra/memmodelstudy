...
struct SPSCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        // faulty: stale locals reused
        static thread_local bool cached = false;
        static thread_local std::size_t tail_val = 0;
        static thread_local std::size_t head_val = 0;
        static thread_local std::size_t next_tail = 0;

        if (!cached) {
            tail_val = LOAD_TAIL(this, rl::memory_order_relaxed);
            head_val = LOAD_HEAD(this, rl::memory_order_acquire);
            next_tail = (tail_val + 1) % QUEUE_SIZE;
            cached = true;
        }
