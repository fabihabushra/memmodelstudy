...
struct SPSCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        ...
        std::size_t head_val = LOAD_HEAD(this, rl::memory_order_relaxed); // faulty
        ...
        STORE_TAIL(this, next_tail, rl::memory_order_relaxed); // faulty
        return true;
    }

    bool dequeue(std::uint64_t& value) {
        std::size_t head_val = LOAD_HEAD(this, rl::memory_order_relaxed);
        std::size_t tail_val = LOAD_TAIL(this, rl::memory_order_relaxed); // faulty
        ...
        STORE_HEAD(this, (head_val + 1) % QUEUE_SIZE, rl::memory_order_relaxed); // faulty
        return true;
    }
};
        ...
        if (tid == 0) {
            for (int r = 0; r < ROUNDS; ++r) {
                for (std::uint64_t v = 1; v <= NUM_VALUES; ++v) {
                    SCHEDULE_POINT();
                    while (!q.enqueue(v)) SCHEDULE_POINT();
                    SCHEDULE_POINT();
                }
            }
        } else {
            for (int r = 0; r < ROUNDS; ++r) {
                for (std::uint64_t expected = 1; expected <= NUM_VALUES; ++expected) {
                    ...
                    SCHEDULE_POINT();
                    while (!q.dequeue(value)) SCHEDULE_POINT();
