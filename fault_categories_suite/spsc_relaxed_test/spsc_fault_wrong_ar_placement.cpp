...
struct SPSCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        std::size_t tail_val = LOAD_TAIL(this, rl::memory_order_acquire);   // faulty placement
        std::size_t head_val = LOAD_HEAD(this, rl::memory_order_relaxed);   // faulty placement
        ...
        STORE_TAIL(this, next_tail, rl::memory_order_relaxed);              // faulty placement
        return true;
    }

    bool dequeue(std::uint64_t& value) {
        std::size_t head_val = LOAD_HEAD(this, rl::memory_order_acquire);   // faulty placement
        std::size_t tail_val = LOAD_TAIL(this, rl::memory_order_relaxed);   // faulty placement
        ...
        STORE_HEAD(this, (head_val + 1) % QUEUE_SIZE, rl::memory_order_relaxed); // faulty placement
        return true;
    }
};
