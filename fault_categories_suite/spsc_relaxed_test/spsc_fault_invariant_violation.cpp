...
static std::size_t occupancy(std::size_t head, std::size_t tail) {
...
struct SPSCQueue {
    ...
    bool enqueue(std::uint64_t value) {
        ...
        // faulty: corrupt tail with an out-of-range value
        STORE_TAIL(this, QUEUE_SIZE + 1, rl::memory_order_release);
        return true;
    }
    ...
    bool dequeue(std::uint64_t& value) {
        ...
        value = LOAD_SLOT(this, head_val % QUEUE_SIZE);
...
struct spsc_test : rl::test_suite<spsc_test, 2> {
    ...
    void check_invariants() {
        std::size_t h = LOAD_HEAD(&q, rl::memory_order_relaxed);
        std::size_t t = LOAD_TAIL(&q, rl::memory_order_relaxed);

        RL_ASSERT(h < QUEUE_SIZE);
        RL_ASSERT(t < QUEUE_SIZE);

        std::size_t occ = occupancy(h, t);
        RL_ASSERT(occ <= QUEUE_SIZE - 1);
    }
